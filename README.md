...
현재 저장소는 MVP 핵심 흐름 대부분을 포함합니다.

- Go 기반 Signaling/Relay 서버 골격
- Go 기반 PC Agent 오케스트레이션 + Cursor 브리지 stdio RPC 연결
- TypeScript 기반 Cursor 브리지 계약/프로세스 서버/extension runtime helper
- WebRTC Peer + SignalBridge + Agent P2P 제어 경로(Envelope) 라우팅 + ACK retry/backoff
- Flutter 모바일 앱(Prompt/Review/Status) + direct signaling/WebRTC control path
- 공통 프로토콜/데이터 모델 문서
...
adapters/
  cursor-bridge/ # TypeScript 브리지 계약 + stdio bridge 서버 + extension runtime helper
mobile/
...
실제 Cursor extension 쪽에서는 `createCursorExtensionRuntime()` 또는 `serveCursorExtensionBridge()`를 사용해 같은 계약을 extension host 안에 올릴 수 있습니다.
